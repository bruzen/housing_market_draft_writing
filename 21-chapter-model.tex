...
*** QUESTION: DOES IT MAKE SENSE TO FLIP THE ORDER OF THE CHAPTERS IN LIGHT OF THIS? % Maybe start with rent, because we are introducing modern urban rent theory, then flip space and growth so we introduce the city first, then agglomeration effects, then finacialization?
...
Following the Alonzo model \cite{alonzoTheoryUrbanLand1960}, (\cite{alonsoLocationLandUse1964}) firms are located at the centre of a circular city, the central business district. Residents are distributed across space, can take jobs, and commute to work at the center. 
...
There is an agglomeration effect, which means firms can also produce more goods by operating in a city with more people, as introduced in Chapter \ref{chapter-growth} on growth.,   %(prior section explains and justifies). 
...
% To simplify analysis, we assume that land outside of the residential limit is costless, following th common practice of assuming a fixed price for agricultural land \cit{cite-fixed-price-ag-land}. 

\subsection{Labor force}
There are also firms that produce goods to sell. They can produce more goods by hiring additional workers. 
Firms pay a wage premium, $w$, over the subsistence wage to attract workers \footnote{
See parameter Appendix section \ref{section-wage-premium} for a discussion of the empirical literature on the \gls{urban wage premium}.}. 
When workers take a job, they give up the subsistence income and instead receive the wage from their employer. % When workers take a job, they  receive, in addition to the subsistence wage,  the wage premium  from their employer. 
The total wage employers pay is the subsistence wage plus the urban wage premium  $\psi + w$ required to attract workers.
...
Agents work if the wage premium is greater than the cost of travel. % Living close to work has value to workers because it saves the cost of transportation. 
The higher the wage, the farther workers can travel for work. 
The \gls{urban wage premium} and transportation costs thus determines both the radius of the circular city and the size of the labour force.   %?There is no fixed boundary and the size of the city is determined by the utility that can be achieved in competing regions of competing for labour.
...
This is the standard urban population equilibrium condition/\gls{migration equilibrium}

The radius of the city, or  \gls{commuter shed} is $\frac{w}{{c}}$, where ${c}$ is the the cost of transportation per unit distance.
Given a uniform lot size $s$, with one worker per unit land, the labour available in the \gls{circular city} is then the area of the circle divided by the lot size: % Do we want to call it a \gls{radial city} ?
\begin{equation}
%=  \frac{\pi}{s}(c^{max})^2	
...
\end{equation}
% Note, this increases with the square of the wage. 
This is the equilibrium urban labour supply curve. To get wage for s given city size, we can also write the inverse labour supply function:
\begin{equation}
...
To introduce  the productive nature of cities, we incorporate Jacobs-style agglomeration economies (\cite{beaudryWhoRightMarshall2009, vanderpanneAgglomerationExternalitiesMarshall2004, jacobsEconomyCities1969}) , using an approach similar to how the \gls{Solow-Swan model} incorporates labour-augmenting technical change. %, using a simple Cobb-Douglas production function. 
We incorporate the estimated scaling relationship in a \gls{Cobb-Douglas} production function. This allows us to focus on the results of agglomeration in the urban system. % rather than specifying the system of firms that transmit the effect. 

The \gls{agglomeration effect} means urban output is more than proportional to urban population: 
 \begin{equation}
 Y\propto N^{\beta}
...
Following \gls{neoclassical distribution theory}, this implies that the wage premium is increasing in the same way: % USES NEOCLASSICAL DISTRIBUTION THEORY TO GET THE WAGE INCREASE
\begin{equation}
\omega\propto N^{\beta-1}
\label{eqn-wage-population}
\end{equation}
The scaling result at the level of the city allows us to incorporate the agglomeration effect in a \gls{circular city} model, building the production function into a standard Alonso-style model of an urban economy, and treating the city as having a single firm facing a fixed price for output. 

Firms sell the goods they produce into a commodity market. For simplicity, firms produce a variety of perfectly \gls{substitutable} commodities which are exported and locally consumed at a fixed price in a large market. Demand for the urban product is perfectly elastic which means producing more won't affect the product's price; and there are decreasing returns to scale, which means each new worker increases output by less than the previous worker. 

MOVE DOWN?
The \gls{agglomeration effect} means that although individual firm returns to scale are declining, the city can experience increasing returns to scale in the utilization of human capital. % NEED? Individual firms have decreasing returns, but the presence of agglomeration economies external to firms but internal to the city gives the urban economy as a whole increasing returns to scale .}.
...
\label{Eqn:Prod1}
\end{equation}
where $n_i$ is labour, or the number of workers at the firm, the labour, and $n$ is the urban population. The agglomeration factor increases with population. It multiplies labour because agglomeration scales the productivity of workers. 
...
%%%%%%%%%. ***WHY
If $\beta=1-\alpha$, this is a \gls{constant returns to scale} (\gls{CRS}) production function. Without agglomeration effects, $T(n)=1$,  Then  \textbf{$\mathbf{L(n) = T(n) n}$}  WHAT IS T, WHAT IS THIS TELLING US?
Without agglomeration effects, $\Lambda(n)=1$,  Then  \textbf{$\mathbf{L(n) = T(n) n}$} 

Firms will purchase the time of workers to capture the product of their effective labour  and enjoy the product of \gls{effective labour}. 
If labour markets are \gls{competitive market}, we set die${Y}{L}=w$.

% Labour always depends on the human capacity, technology, 
...
\subsection{Economically warranted rent}
Rising population and productivity generates an economic value for land that provides proximity to the production center. 
% In our model, non-urban landowners are those who live too far from the urban job center to justify commuting.  Agents join the urban market by adding their names to the firm's list of job applicants when the rent  on the marginal unit of housing or land exceeds the transportation cost. 
...
\subsection{Relation between urban productivity and land markets}
Our goal is to look at the relationship between urban production/labour markets, agglomeration, and land/housing markets. To explore this relationship, we integrate the model of production developed above with a spatially explicit agent-based land market.

OR? To explore the relationship, we integrate the urban production model  into a spatially explicit land market with financialized investment. 
...
Individuals and institutions buy and sell properties given their individual goals, resources and available information. We integrate this housing market with % within this model of individual and institutional actors in a spatially explicit property market, a model of production and employment.
