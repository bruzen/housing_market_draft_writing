...
Properties are exchanged on a market at a given price.

exchanged through a bidding process, 
that biding 

reflects-- 
doesn't always reflect market rates in the lit
to link with productivity, we need a model that allows for speculation in exchange, and a realistic/plausible bidding process, but also reflects underlying economic value driven by urban productivity. 
...
This requires distinguishing between a number of concepts, outlined in Table
\ref{table-price-notation},

INTRODUCE ALL TERMS. any value can be expected if it's forecast. 

capture markets

We need several kinds of rent/price..
warranted is xyz
the actual  bid price, and what people will be willing to pay, accept
and the realized price at which a property is exchanged on the market. 
...
\Gls{locational services} are, on an annual basis, the rent premium, $w$, minus the transportation costs, $c$, for a property a given distance, $d$, from the center. That is the value of living in a particular location, close to work:
\[\omega- {dc}.\]
...
LOGICAL PLACE TO START SINCE WE EXPECT IT TO GO TO THE NET RENT..
Serves as a clarification of the underlying machinery
and a justification for where you start the search process when you initialize the model.
...
As a convenience, for the analysis here, we assume the warranted rent is paid as the market rent. This is easy to vary in the model as part of future experiments, with a range of decision rules. 

It is an important assumption in the analysis because it connects the work with the formal analytic tradition and is in an important sense what makes this a classical model MOVE DICSUSSION HERE...
...
The price analysis that follows assumes that the actual market rental price is the warranted rental price. Rents can diverge from warranted rents. In practice, some people may pay more or less than the warranted rent. They may draw on family assets, assume debt, spend more on amenity, etc. Rents are, however, limited by what tenants can pay, and that, in turn, is limited by urban wages. WHERE DO WE MAKE THAT ASSUMPTION.

Building this relationship into the model is a core conceptual contribution of this work. It makes it possible to explore formally the relationship between the scaling of urban productivity, and the potential for financialized ownership to extract that value, pushing those who contribute to creating growing wealth, to a subsistence frontier\footnote{***WHERE IS MAIN DISCUSSION OF THIS? ** RENT CHAPTER? .. %METHOD/model/intro section - ref section 
...
METHODOLOGY CHAPTER? The warranted rent %is a frontier the market pushes the agents towards.  
