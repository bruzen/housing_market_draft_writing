...
See parameter Appensix section \ref{section-wage-premium} for a discussion of the empirical literature on the urban wage premium.}. 
When workers take a job, they  receive, in addition to the subsistence wage,  the wage premium  from their employer. 
...
?There is no fixed boundary and the size of the city is determined by the utility that can be achieved in competing regions of competing for labour.
...
Agents work if the wage premium is greater than the cost of travel. %The raidus of the commuter shed is thus,  %The farthest workers will travel to work is thus 

The radius of the commuter shed is the maximum incentivized commute, $\frac{w}{{c}}$, where ${c}$ is the the cost of transportation per unit distance.
Given a standard lot size $s$, the labour available in the \gls{radial city} is thus: % Do we want to call it a radial city?
\begin{equation}
%=  \frac{\pi}{s}(c^{max})^2	
...
\end{equation}
which increases with the square of the wage.  DEFINE $s$? 
...
The farthest workers will travel to work is thus $\frac{w}{{c}}$, which defines the radius of the commuter shed. Thus a worker, located at a distance $d$ from work, paying as much as $w- {dc}$ in rent, would still choose to work, and the maximum distance that workers will commute is the radius of the commuter shed. Given a uniform lot size $s$, with one worker per unit land, the labour available is the area of the city. In the circular city, this is the area of the circle divided by the lot size
\begin{equation}
                 L%=  \frac{\pi}{s}(c^{max})^2	
...
% $w^n=\omega-{c} d$ & Wage  premium net of transportation costs \\
