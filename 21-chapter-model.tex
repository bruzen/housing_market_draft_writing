...
Demand for the urban \gls{product} is \gls{perfectly elastic}, so the price remains constant. DUPLICATE

Firms produce more goods by hiring additional workers. There are \gls{decreasing returns to scale}, which means each new worker increases output by less than the previous worker did. 

To attract workers, firms pay an \gls{urban wage premium}, $w$, over the subsistence wage. 
...
the total wage employers pay is the \gls{urban wage}, which is the subsistence wage plus the urban wage premium $\psi + w$.
...
% This model uses a similar functional form to look at the effect of population density increasing % productivity. %how density increases in in  % It models how population increases productivity. 
 ...
 We use a three factor model of \gls{production}, where production, is a function of capital and labour as well as a third factor, a Solow-Swan style term for labour augmenting technical change.
 % the productivity of labour, rather than technology growing productivity over time. 
 With agglomeration in place of  technology $A(t)$, 
...
\label{Eqn:Prod1}
\end{equation}
where $K$ is capital, $\Lambda(n)$ is \gls{labour augmenting agglomeration},  $\alpha$ and $\beta$ are elasticity of output with respect to capital and labour, and  $n_i$ is the \gls{urban labour supply}. Then $\Lambda(n)n$ is \gls{effective labour}. CHECK WHAT FORM WE WANT AND MAKE SURE EQUN IS CONSISTENT 
...
WHAT DO WE WANT TO ADD BACK IN/CLARIFY FROM WHAT'S BELLOW? WHAT DO WE WANT TO MOVE TO THE GROWTH CHAPTER OR AN APPENDIX?
...
Rising population and productivity generate an economic value for land that provides proximity to the production center. 
...
To do this, we build a simplified model of a housing market.
...
The market is made up of a set of agents who engage directly in the market, including buyers, sellers, renters, investors, developers. Urban productivity %and amenity
drives land values through the housing market. Where there is a strong labour market, residents have a greater willingness to pay for housing \cite{productivity_price_link}.
Others affect the market indirectly, including policymakers, activists, developers, and others, % and those who affect the policy contexts (such as activists, developers, etc), and labour.
and there is a a layer of policy. % and rules that determine the 
%that shapes land value. 
For example the city zoning rules, allowing certain uses and disallows others, % and charges taxes and fees which are factored into the cost structure.
shapes the land value.

In the simplified market model, %  owners sell when the retire.
we integrate the urban production model into a spatially explicit land market with financialized investment
