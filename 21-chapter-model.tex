...
a*b*psi % #   annual operating cost in dollars  
  ...
  _
c   %# annual tax rate on rent and home  eg  12 mills 
    %# PVc_T =  present value of the tax rate over T years
    %# 0.012/.r - (0.012/r)*delta_T
    %#       = 0.05195372
...
Doing the derivation to check my logic. omega, psi and a are constants that can be factored out in the following
\begin{align}%
    tax&= \sum_{t=0}^{T-1} \delta(t) \left(c*\omega + c*a*\psi \right)\\
        ...
        &= c(\omega + a*\psi) \left(\frac{1+r}{r}  - (\frac{1}{1+r})^{T+1}\frac{1+r}{r} \right)\\
        &= c(\omega + a*\psi) \frac{1+r}{r}*\left(1  - (\frac{1}{1+r})^{T+1} \right)
...
The social structure of the model begins with  three spatially segregated ``classes.'' Capitalists live in some spaceless utopia, urban workers who commute and earn wages $\psi + \omega$ in the urban commuter-shed, and  rural landowners who may choose to move to the city. %There may be a band surrounding the city or persons who do not commute but enjoy urban consumption amenities. 
...
\begin{equation}
%=  \frac{\pi}{s}(c^{max})^2	
 L = \frac{\pi}{s} \left(\frac{w}{\tau}\right)^2
   = \frac{\pi}{s \tau^2} w^2,
...
\begin{equation}
			...
			=\frac{\pi}{\tau^2 s} w^2, \label{eqn-labour-supply2}
\end{equation}
...
%\begin{equation}
%	w= (\frac{ \tau^2s}{\pi})^{0.5} L^{0.5},	%\label{eqn-inverse-labour-supply}
%\end{equation}
