...
\label{Eqn:Prod1}
\end{equation}
where $K$ is capital, $\Lambda(n)$ is \gls{labour augmenting agglomeration},  $\alpha$ and $\beta$ are elasticity of output with respect to capital and labour, and  $n_i$ is the \gls{urban labour supply}. Then $\Lambda(n)n$ is \gls{effective labour}. 
MAKE SURE EQUN IS CONSISTENT WITH GROWTH CHAPTER DISCUSSION. 
...
Our goal is to look at the relationship between urban production/labour markets, agglomeration, and land/housing markets. To explore this relationship we introduce a land market.

% Agents purchase homes though a housing market.
To do this, we build a simplified model of a housing market.
In practice, housing is exchanged on market. The product is land, at a particular location, with various buildings and features. %properties on them. 
Land value is affected by various elements including proximity to natural features like lakes and mountains, proximity to human-built feature like communities, businesses and amenities, as well a developments and additions on the property itself. 
The market includes is made up of a set of 
agents who engage directly in the market, like buyers, sellers, renters, investors, and developers and others who effect the market indirectly, policymakers, activists, neighourhood and industry groups. % and those who affect the policy contexts (such as activists, developers, etc), and labour.
and there is a a layer of policy. % and rules that determine the 
that shapes land value. 
For example the city zoning rules, allow certain uses and disallows others. % and charges taxes and fees which are factored into the cost structure.
...
we integrate the urban production model developed above into a spatially explicit land market with financialized investment.

Rising population and productivity generate an economic value for land that provides proximity to the production center. 
The rents go to the owners of a given property. If workers own their own homes, rents go to them. If others own the land, they can extract the rent. 

Individuals and institutions buy and sell properties given their individual goals, resources and available information. We integrate this housing market  within this model of individual and institutional actors in %a spatially explicit property market, a model of production and employment.
