...
The main goal of this research is to add a financial system in a spatial urban model %to the simple Alonzo model in order 
...
Figure xx shows the model flow/logic. ELABORATE.
...
\subsubsection{Prices} \label{sec-prices}
...
We will call the economically justified price, given the urban productivity premium, the \gls{warranted price},  $P_W$.

The \gls{market price}, the price at which a property is exchanged on the market, $P_M$. 

Values may not be known, so in order to make decisions, agents must form  \glspl{expectation} or   predictions of the as yet unrealized future market price. Agent $i$'s expected market price is  $P_{M_i}^e$, which may include individual information, biases, preferences, etc.
...
\subsubsection{Warranted Rent
Just as with prices, there  is economically \gls{warranted rent}, $\mathcal{R}_W$, and \gls{market rent}, $\mathcal{R}_M$. The {warranted rent} is the maximum rent that is economically justified. It is based on the services a property offers.
...
the explicit treatement of rents and the relationship between the productivity \gls{premium} and the \gls{subsistence wage}
