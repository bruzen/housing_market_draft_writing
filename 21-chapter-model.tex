...
There is an agglomeration effect, which means firms can also produce more goods by operating in a city with more people, as introduced in Chapter \ref{chapter-growth} on growth.

We assume workers receive a \gls{subsistence wage}, $\psi$, in the countryside, which could come from work in the local community, living off the land, family support, social support, or something else. %[MAYBE This follows xyz's approach, and makes it possible to explore resident's choice to work]. 
...
There are also firms that produce goods to sell. They produce more goods by hiring additional workers. 
To attract workers, they pay an \gls{urban wage premium}, $w$, over the subsistence wage\footnote{
...
the total wage employers pay is called the \gls{urban wage},  which is the subsistence wage plus the urban wage premium  $\psi + w$.
...
The higher the wage, the longer workers will commute.
The farthest workers will travel  is $\frac{w}{{c}}$, where ${c}$ is the the cost of transportation per unit distance. This defines the \gls{commuter shed}, the radius of the circular city.

Given a uniform lot size $s$, with one worker per unit land, the labour available in the \gls{circular city} is simply the area of the circle divided by lot size: 
\begin{equation}
%=  \frac{\pi}{s}(c^{max})^2	
...
\end{equation}
This is the equilibrium \gls{urban labour supply} curve. It is the standard urban \gls{population equilibrium} condition, also called the \gls{migration equilibrium} \cite{migration_equilibOR_pop_equilib_condition}. It defines the quantity of labour available.

In the agent model model, the \gls{urban labour supply} is an \gls{aggregate} measure that emerges from individual agent's choice to seek work, model rules, and individual firm's hiring behaviour. %and the job search process. 
In the basic model, the outcome is identical to the analytical case.  It diverges as additional decision factors, agents, driving variable, and \gls{stochastic} elements enter the model. %\footnote{See the discussion of model extensions in Appendix \ref{appendix-future-work}.}.
...
The \gls{agglomeration effect} means urban \gls{output} is more than proportional to urban population: 
 \begin{equation}
 Y\propto N^{\beta}
 \label{eqn-production-population}
 \end{equation}
where $\beta$ is the EXPLAIN, and $\beta >1$.   

According to \gls{neoclassical distribution theory}, the firm maximizes profit by setting the marginal value of the product of each \gls{factor of production} equal to the unit cost per factor. This means the wage premium increase, in the same relation to population, as \gls{productivity} does:
\begin{equation}
...
% This model uses a similar functional form to look at the effect of population density increasing % productivity. %how density increases in in  % It models how population increases productivity. $\Lambda(n)n$ is  ``effective labour'' 
 
 We use a three factor model of production, where production, is a function of capital and labour as well as a third factor, a Solow-Swan style term for labour augmenting technical change:
 % the productivity of labour, rather than technology growing productivity over time. 
 With agglomeration in place of  technology $A(t)$. Following Equation \ref{}, 
 % and it's development in Chapter \ref{chapter-growth}, 
 output is: 
...
\label{Eqn:Prod1}
\end{equation}
where $K$ is capital, $\Lambda(n)$ is \gls{labour augmenting agglomeration},  $\alpha$, $\beta$, and $\gamma$  are elasticities with respect to capital, labour, and agglomeration respectively, and  $n_i$ is the \gls{urban labour supply}. CHECK WHAT FORM WE WANT AND MAKE SURE EQUN IS CONSISTENT 
*** THIS EQUATION SHOULD HAVE GAMMA?
...
\begin{eqnarray}
 ...
 \label{Eqn:Prod2}
\end{eqnarray}
EXPLAIN WHY THIS IS A NATURAL FUNCTIONAL FORM
...
Demand for the urban \gls{product} is \gls{perfectly elastic}, so the price remains constant, and there are \gls{decreasing returns to scale}, which means each new worker increases output by less than the previous worker did. 
