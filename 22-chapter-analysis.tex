...
\begin{enumerate}
    \item total rent.  $N(\omega +\mathbb{A}^{net})$ two parts  separately
    \item urban-generated surplus
    \item  Wage $\omega$
    \item population of workers $N$
    \item city extent $\omega/c$
    \item GDP Growth $N(\omega +\mathbb{A}^{net}) + r\Delta k$
    \item Amentity $\mathbb{A}^{net}$. fixed. We want total amenity
    \item dissipated rent $\sum_i^N c_id_i$  
    \item avaailable rent.  $sum_i^N(\omega -cd +\mathbb{A}^{net})$
    \item Rent captured by the finance sector
    \item Sum of current mortgages outstanding. $\sum_i^N M_{i,t}$ (ownership)
    \item  Sum of current mortgages outstanding plus value of property owned is share of city-owned
    \item Sum of current mortgages outstanding plus net rent. $\sum_N M_{i,t}$ 
