...
The purpose of this model is to provide a representation of city development financialization, where financialization in this context means the purchase of land and housing by owners of capital. More specifically, we seek to understand the links between financialization and the distribution of housing ownership, the capture of rents, and urban productivity. 

The modelling challenge is to combine a theoretically correct model of the spatial and temporal growth of a model city with a model of a housing market in which individual agents buy and sell property, live, work, and retire subject to a realistic financial system. 
...
\begin{figure}
    \centering
    \resizebox{16.5cm}{!}{%
        \input{fig/Kirsten_overview_figure.tex}
    }
...
State variables refer to the attributes of each kind of entity that varies over time. 

%\footnote{In the ODD framework, state variables commonly refers to both stock variables and parameters. Although it is inconsistent with usage in the analysis of dynamical systems, we follow the ODD convention in this section.}.

The entities in our model are the five agents types illustrated in Figure~\ref{fig:Kirsten_overview_figure.}: 
\begin{enumerate}
    \item an investor,  
    \item  a bank, 
    \item a realtor, 
    \item an urban firm,  
    \item land units (properties or lots). Each unit of land has an owner, either resident or non-resident, and a resident worker who may be an owner or a tenant. The population is therefore modeled through individual relationships to specifc properties. Properties are in the city if the resident chooses to commute to work at the center of the city
\end{enumerate}
In Figure~\ref{fig:Kirsten_overview_figure.}, agents are identified by black boxes around the agent name. The first four are representative agents.The residents of our city are modeled as attributes of specific units of land: owner-occupier or tenants or bid on propeties in-migraants who . 
In Figure~\ref{fig:Kirsten_overview_figure.}, agents are identified by black boxes around the agent name. The first four are representative agents. The residents of our city are modeled as attributes of specific units of land: workers or non-workers and owner-occupier, tenants or in-migrants who bid on properties. The city grows when non-worker residents at the edge of the city are induced by a rising wage to begin commuting to the cc\ity center to work.

Combined, these elements provide a simulation of a growing city in a rural region. 

\subsubsection{Parcels of land are individual agents}
We begin with a grid of properties.  Each cell is a computational agent in the class of Land agents. Each cell has an address in a Cartesian space. Each has a set of attributes (state variables in the terminology of the documentation format we are using.) Properties are updated as the model evolves.

The city is the subset of cells - the region - over which workers choose to work at the urban center, located in cell (0, 0). Agents are assumed to travel to the center to work only if the transportation of travel to the center is less than the extra pay earned for working at the center compared to the pay earned in rural occupation (the urban wage premium). Transportation cost is the grid distance from the center (The distance is the sum of the absolute values of the x and y displacements from (0, 0)  multiplied by the transport cost. The transport cost per unit distance is set to ensure that the city that emerges will remain within the boundaries of the grid in the course of a simulation run


To limit computational time, population is modelled coarsely and the size of the grid is restricted.  The density parameter is chosen so that the limited number of cells in the city provides a plausible urban population. Each cell has a single owner but represents a specific number of individuals for the population count and for the labour market. (The density parameter is the simplest version of a density function, which is itself a simplification of a mixed population.
...
\begin{figure}
    \centering
    \includegraphics[width=1\linewidth]{fig/Kirsten_overview_figure.pdf}
\end{figure}
% \centering
% \begin{tikzpicture}
...
% \end{tikzpicture}
% \caption[The platform, the process, and the outcomes]{The major processes: the production sector which forms the platform creating value distributed through rents, the housing market which is the process actors claim social value, and the social outcomes that may emerge. %. Feedback loops are omitted. %{\color{red} CAN WE ELABORATE? The platform, the process, and the outcomes}.
% }
% \label{fig:outcomes}
% \end{figure}
...
% \footnote{The agents are described in the agent section of the code.}
 % entity that includes the agents. % The city administration is not an agent. %  who may choose to work at an urban firm, properties, each occupying a an urban grid space. %, 
...
The state variable (attributes) for persons are property location, initial working period (providing age), savings, ownership status, capital gains tax level, ownership status, and property owned. Mortgage payments or rental payments are calculated to update savings. Each person occupies one property. 
...
\subsubsection{Firm}
A representative firm is the sole member of the class Firm. The state variables for the firm are capital stock and workforce. The number of firms also adjusts. These variables evolve from current values toward target values according to partial adjustment rules. The adjustment rates are specified in the code.  

The firm does not set wages or output prices: it is a price taker. Although we employ a single representative firm to determine labour demand,  we imagine that the wage adjustment process occurs in a decentraliΩed and competitive market with many firms, each forced to adjust their own wage offer in the face of excess demand or supply. This decentralized market response is captured in our model by a partial adjustment mechanism. 

At any price, wage and cost of capital, the firm identifies target employment and capital stock. The target is calculated using a rule derived from standard neoclassical firm theory. The firm then adjusts its current toward the target. 

%The production function is specified over  $N$, $A$, $alpha$, $beta$, and $gamma$. 
The target employment for the firm times the number of firms gives us the aggregate target labour. We term this value labour demand.  %Since the price of output is fixed, we can ignore output. %With perfectly elastic demand as assumed, the firm can sell any amount at the fixed price. 
The current population of the city is determined by agents near the edge of the city (marginal agents) deciding to begin or stop work in the city or based on the previous period's wage. When discussing the labour market discussion, we call this the labour supply. 
...
In the production function, $N$ is the agglomeration population, which may or may not be the total urban workforce. It is computed from the number of cells that are urban, the urban seed population, and density, and adjusted to account for the fact that not everyone works for the firm. 

The firm also has a target capital stock calculated as the optimal value at current prices.
