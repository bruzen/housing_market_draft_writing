...
\begin{enumerate}
    ...
    \item A model of the institutional mechanisms by which some agents capture the rents through property markets. %financial and institutional structure linking them, which defines who gets what share of the surplus through ownership, taxation, etc.
\end{enumerate}Each component is extensible, % is designed  to accommodate a range of extensions, 
and the model is modular enough that any given segment may be removed and replaced with another, perhaps more detailed, implementation of the sector. 
In the following sections, we describe possible extensions to each of the three components. % that open a set of interesting questions. 
...
% {Equation 8.21 to examine the effect of transaction costs It is useful to review some of the strong simplifications of the current model. 
% }
...
% We look forward to relaxing many of the assumptions to allow us to explore additional questions.\footnote{We want to emphasize that, although our investigation has focused on financialization and one of the most important results is the potential importance of linkages between financialization and productivity, we do not intend to undertake empirical research on those linkages. The empirical research is important, but it is not our field.}  
...
% %% NO PAGE NUMBERS
% \begin{figure}\thisfloatpagestyle{empty} 
% \vspace{-4.5cm}
% \begin{adjustwidth}{-0.24\textwidth}{-0.2\textwidth}
% \centering
% \includegraphics[scale=.15]{fig/extensions3.png}
% \end{adjustwidth}
% \caption{Extensions}
...
% Full-page image with invisible text to push it down
\noindent
\begin{minipage}[t][\textheight][t]{\textwidth}
  % \vspace*{\fill}
  \vspace{-3cm}
  \begin{center}
  \includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{fig/extensions3.png}
  \captionof{figure}[Model extensions]{Model extensions.}\label{fig-extensions-logic}
  \end{center}
...
Cities also contain a rich variety of housing forms and neighbourhoods and the varied populations that occupy them. Separable from production, it is possible to introduce variation of housing form and density.  % Many questions about who gets what housing arise at this point. 
...
Homeowners in addition have equity %$E=P-M$ 
in their homes. % ({ Should newcomers also have equity? or is it built into the savings. Clarify this.} 
...
% \end{center}
% \caption[Financialization changes ownership but does not expand the housing stock]{Financialization expands investor ownership but does not expand the housing stock. The white area represents the part of the housing stock that is owner-occupied and not currently mortgaged. Yellow represents privately owned homes for which ownership is partially held by mortgage lenders. Dark grey represents the portion of the housing stock already owned by investors and light grey represents the part of the housing stock undergoing financialization. The process of financialization is orthogonal to the process of creating new housing.} 
% \label{fig-financialization-expansion}
% \end{figure}

\begin{figure}[ht]
\begin{center}
\resizebox{0.68\textwidth}{!}{\input{fig/financialization-expansion.tex}} % Adjust width as needed
\end{center}
\caption[Financialization changes ownership but does not expand the housing stock]{
  Financialization expands investor ownership but does not expand the housing stock. The white area represents the part of the housing stock that is owner-occupied and not currently mortgaged, yellow privately owned homes that are partially mortgaged, dark grey the portion already owned by investors, and light grey the part of the housing stock undergoing financialization. Financialization is orthogonal to housing creation.
}
\label{fig-financialization-expansion}
\end{figure}
...
The conclusion drawn from both theory and our simulation is consistent with what has already happened in the Canadian Housing market. Recent data from Statistics Canada \cite{fontaineResidentialRealEstate2023} suggests people who own more than one property in Ontario make up more than 25\% of buyers in the province. %(The proportion of investors among owners varied from 20.2\% in Ontario to 31.5\% in Nova Scotia.)
...
dividing extensions by the three main elements of the model: the model of the city and it's land market, the model of firms and production, and the model of the institutional and legal structures by which some agents may claim a share of the value created in the city.\footnote{The extensions detailed here also combine. Models are by nature combinatoric: every added element involves making a choice among alternative assumptions and implementations. Each extension is potentially as complex as the core model, and each introduces additional assumptions. A model incorporating $n$ binary choices is one of an implicit family of $2^n$ alternative models.}  % These extensions would substantially enrich our understanding of the urban system and the process of the financialization of housing.
