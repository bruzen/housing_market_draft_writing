...
It is useful to review the main features of the current model. A representative  urban firm %central place 
pays a uniform wage, $w$ to all employees. Employees  have identical preferences and transportation costs. %$where $w$ is an attribute of individual residents.
Residents purchase or rent equal quantities of land at differing locations $l$ for identical housing.  There are transportation costs $T$ that depend on distance from the central place, so land close to the central place is more attractive than land farther from the central place.  The equilibrium concept is that a market with identical individuals with identical incomes and transportation costs will result in identical utilities. The result is that land rent must decline with distance from the central place to offset rising transportation cost. The size of the city is determined by the wage, transportation cost and lot size. %Income and transportation costs will interact with lot size. 
...
This simple case allows us to focus on the general, and neglected, distributional features of this class of models. We look forward to incorporating a number of extensions that will allow us to explore additional questions.\footnote{We want to emphasize that, although our investigation has focused on financialization and one of the most important results is the potential importance of linkages between finacialization and productivity, we do not intend to undertake empirical research on those linkages. They are important, but empirical research is not our field.} Three of the most immediate and promising extensions involve adding attributes to agents.\footnote{In code, the extensions in this group require simple additions to agent descriptions.} Another group of three extensions add structural elements to the model.  Combined with the extensions in first group, they form a set of related projects. The augmented agents can be incorporated in the structural models and should be completed first.
...
Introducing multiple wages, amenity, and differing densities are relatively simple. Wages can be treated as an aspect of the individual the way we have treated savings. Amenity and density requires us to add attributes to locations and preference structures to individuals, which is slightly more complicated. Much more time is likely to be need to analyze the results of different distributions of attributes. 
These changes are relatively minor although they add a great deal of granularity.

Sorting by wealth, income and preference is the expected result, but our model will allow us to  see if financialization affects the sort and whether public policies can have positive effects
