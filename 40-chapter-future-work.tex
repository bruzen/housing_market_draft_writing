...
Equation 8.21 to examine the effect of transaction costs
It is useful to review some of the strong simplifications of the current model. \begin{itemize}
...
\section{Elaborating the agents}
There are three extension that are technically  minor although they add a great deal of granularity. Each involves adding attributes to on or more classes of agents and adjusting the relevant behavioural rules. They are
\begin{enumerate}
    \item introducing workers with different wages. It is not necessary to generate the differences out of the production sector to see how different wages affects the spatial distribute of workers.Sorting by income is the expected result, but our model will allow us to  see if financialization affects the sort and whether public policies can have positive effects
 
    \item introducing differences in amenity. This involves  adding attributes to properties and preferences over the attribute to the persons.

    \item Introducing housing density variation. As with amenities, density variation density requires us to add attributes to locations and preference structures to individuals. As with amenities we expect sorting by income and preference in this case as well but density is an urban attribute that is important for many reasons, not least because affects population and is therefore going to interact with productivity, and be amplified through the Alonso-Jacobs feedback loop. 
    
\end{enumerate}

Introducing multiple wages, amenity, and differing densities are relatively simple. The resulting sorting by wealth, income and preference is of considerable interest for social policy. The extensions will allow us to test whether financialization affects the sort and whether public policies can have positive effects

 
\section{Structural additions}

Adding  structure to the model involves more challenges. Three directions seem most promising.
\begin{enumerate}
    \item Embedding the city in a system of cities will let us  explore a number of important questions. How does the city connect with the larger system of cities, for example, and does a dominant city emerge? Tow what extent will wee see specialization? Wehn are systems of cities stable? To explore these and other questions we would simply clone the model and connect the clones with population flows that respond to local variables. 
    
    \item Introducing a housing production sector with land developers and home construction is a larger task. Economic dictates and building codes and master plans regulate which land is developed at what density and when. Considerable work is underway among collaborators at Waterloo, raising the possibility of productive synergies.
...
% \section{My research agenda - dreamland}

% Housing supply-production
% Nonlinear dynamics

% Inequality combined with other barriers
...
\subsection{Amenity}
In our base model, an urban wage premium is the only labour attractor. Transportation costs to the urban center determine land values. Effectively in our base model, we have set the level of amenities to zero to focus on the productivity effects.
...
We can introduce a housing production sector. This extension appears on the right side of Figure~\ref{fig-extensions-logic} linked to the banking sector. It requires adding a dynamic housing stock, either directly in the equations or explicitly by modelling the actions of agents acquiring and developing land. % to subdivide and develop. 
...
\subsection{Density, housing form, and neighbourhood amenity}
...
The scaling literature also provides relationships between density, population and infrastructure cost that we can explore in the same way. In the scaling literature, these relationships are increasingly theorized in terms of network effects, which is consistent with the Jacobs analysis and the more recent neoclassical growth modeling.
...
In this chapter we've discussed ways the model can be extended and suggest further hypotheses that we  would like to test using our model. We would argue that it's likely none of them would change our qualitative results greatly, however each would deepen our understanding of mechanisms and of the detailed impacts. 

We divide extensions three broad categories of extensions. The main questions for immediate future work, connections with existing bodies of work, and the extensions needed to build out the base of this model and make it more rigorous. 
