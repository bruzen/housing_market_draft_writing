...
We have sharply restricted our model in order to focus on one process of significance, financialization and its effects, but simple extensions, the model is capable of generating testable empirical hypotheses on a series of policy issues. Each component is extensible, % is designed  to accommodate a range of extensions, 
...
In each case it is also possible integrate data more systematically, or to more thoroughly explore the static and dynamic properties of the model. In the following sections, we extensions to each module that open a set of interesting questions. % detail several promising extensions to each module.
...
The degree of spillover will be sensitive to the relative elasticities of demand and supply  for labour, land, consumption, and local densities. These leakages will reduce the amount of the social gain captured as land rent. % Without building in elasticities we cannot make predictions for specific cities or situations. % We  have not built the elasticities into our model so we 
...
Embedding the city in a system of cities makes it possible to explore questions such as: % will let us  explore a number of important questions. 
...
% Firm behavour could be modelled explicitly. 
...
As a step towards a full agent based model of firm behaviour we could model labour markets with multiple firms using an extension of the equilibrium approach implemented here. % is a natural way to specify the model more completely,  but 
...
This leads to consideration of firms that produce different products, some for export some for the local market, and to multiple types of labour. This kind of model could allow individual wages to be linked to the agglomeration of other workers. % - say 
