...
Financialization is at the heart of this. To introduce how we're thinking about financialization, we’re gonna talk about what financialization is %specifically, %CUT and how it is created in. It is done accomplished in the housing market like 
how it is accomplished
and the specifics of what financialization looks like in the housing market.


Essentially, in Financialization, %we have a process underway in which 
...
classical economics developed during during the colonial period. With the emerging massive concentrations of wealth, it was %Moving into industrialization was 
...
%CUT OK so all that matters here is that the structure of the economy in Europe, which they were looking at was based on landowners, owning land and farmers working the land and then they would sell the product that’s what’s key to this red concept this was also developed in the context of increasing a quality as colonialism brought in flows of wealth so there was a Ricardo was writing in the context of a still largely feudal structure with colonialism shifting equality, so there was this increasing focus on inequality, but the system he was looking at was not quite yet it was soon to be transformed, but not quite yet transformed by industrialization

Classical rent theory was developed by Ricardo to talk about the value of the land, which was extracted by these landowners,% so at the time farmers were paid to work the land or something so at the time landowners would own the value of the land, and 
the value of the land was set according to how far it was from the market, %CUT  so rent theory was about how then you can just take like a description I think you’re fairly succinct we just fill in the description so basically fill in the description very precisely here of land value being like, and it’s based on the 
%CUT so you have the story of the carter and the car has to transport at the marginal you have 

I'LL PROBABLY FILL IN A BIT MORE ABOUT the carter story, and the extensive margin and  the intensive margin. 
%CUT Then you go after filling in the details of the carter or whatever whatever you say what this 
This gives the basic relationship between location, surplus and the value of land %CUT tells us is that the value of the land is related to its closeness so this gives us a spatial relationship to city centre that we can use when we’re thinking about economics in an urban context and that’s really really important to what we’re doing here, right
...
This is the marginalist approach, in which workers are understood as getting the marginal value of their contribution to production.  % - not exactly what they actually get, but it does give a rule that makes it possible to 
...
% The model that we use this, t
through the Alonzo model, % it's actually has several other names attached to it and Muth Mills came out of the early 60s. urban model of locational rent- that brought this into the city -
that brought  a version of Ricardian land rent back in to understand spatial urban land rents %and become a workhorse model for urban economics.

% Although it's stayed central in planning, rent and space has dropped out of standard economics and finance, in part as production has shifted from agriculture towards first forms of production that require space less explicitly: to factories, and offices.

% A contribution here is we're introducing 
What we do here is introduce space in a new way.

To make the relationships clear,   % It's really simple, really neat, and 
we use a  simple version, a circular city with uniform transportation costs.

Here's the standard picture. There are jobs at the center and at the center people have to pay to get the work so %CUT distance matters to them. 
there's a maximum distance that it's worth traveling to work
that determines how big the city is.

%CUT COULD PLUG IN ANY MAP HERE. And the  interesting thing here is 

%if you live close to the city, you don't have to travel. So you don't have transportation costs and %CUT you really like to, you don't have to pay that cost. So you'd like to be close to the city and that means 
It also shapes land prices. Workers will pay more for properties close to the center of the city to be close to work. %CUT in this model.

In our model %The addition in our model is that 
that means rents can be higher

The willingness to pay of investors depends on the rents they expect to get from the land.
 %CUT That means we can compute it, and we can compute it with different expectations etc.
% - those rents can be claimed by whoever owns the land.

%CUT That also means Okay, but if you're paying more 
that means that whoever owns the land is extracting some of the wage workers are earning in the city.
And there's this structural link between place, productivity and distribution. % that you're earning in the city.

The wealth of the city comes from bringing people together and . % at the center is being captured by that's what land owners okay if the landowners happen to be the the people who are working, then the wealth of the city is being distributed among the people of the city but if the wealth if the land is owned by a separate class of landowners than they are expropriating the wealth they're expropriating, 
...
This is orthogonal to the process of investment and development. %CUT - creation of housing- confounded in the literature, which misses this whole structure of rent - 
...
%MOVE to growth - premium comes from growth - So specifically, what is it that makes people want to move to the city? Well, the city are more productive so the firms pay more than  firm's outside the city do.  We call this an urban wage premium. It's, it's well established in the literature. PLOTS And the wage premium is basically a measure of the extra productivity of the city workers in the city.  It's the wage premium that the landlords are capturing. 

%CUT This is the observation that I found really fascinating. 
%CUT BRIDGES SCALING LITERATURE AND JANE JACOBS URBAN PRODUCTION WITH the classical notion of rent, and the formulation of class Roemer developed an excellent treatment of class rooted in the source of income. 
...
%CUT So part of what we need to do is model 
how value is created in the city. 
%CUT  We know a lot about that. 
...
% So part of what we need to do is model how value is created in the city. WE DO THIS WITH STRONG CONNECTION WITH THE ECONOMICS LITTERATURE SO EACH PIECE CAN BE BUILT BACK. --  TALK A BIT ABOUT HOW IT CONNECTS WITH OTHER LOGICS OF FIRM- AXTEL WAS HERE, HE'S MODELLING FIRMS EXPLICITLY. his history of firm modelling reasonin review..we take theo most classic and mainline formulation of the thoery of the firm- to make watertight connections iwth the litterature. note modular enough could just subsitute out pieces. 
...
% This specific formulation we use is pretty standard, 
the Cobb Douglas production function.  % It looks like this.
...
So what we have to solve for with this equation, in a sense is how you put firms with diminishing returns to scale, into an economy with increasing returns. % (the bigger it gets, the more it grows..)
...
% Now, among the economists, this problem that that the macro economy seemed to have increasing returns and they thought that all the pieces of it had Decreasing Returns was a problem. And in the 60s, you started to get what's called growth theory, neoclassical growth theory emerging.
...
% That gives us the ownership story. 
% We can connect these things and as what happens to the structure of ownership in the city. 

% We need one more piece, which is to understand how this financialization affects productivity.
% we’re going to be adding an extension which allows us to look at how it affects productivity and
% in order to do that we need this piece about what are the channels by which financial ation of the housing project house the housing market which is the transfer of value into the hands of financial actors is affecting productivity and we’re gonna talk about channels through that so we’re gonna lay out some possible channels through which that connection or that affect might be happening
...
% Jacobs model, and that was the first thing I described. 
...
So that's our financialization component. And the  story %is really almost as simple as this: 
