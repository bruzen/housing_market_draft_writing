...
\begin{frame}{}
    \begin{tikzpicture}
        ...
        \draw[red, line width=.5mm, opacity=.6] (-2.2, 4)  rectangle (2,1.5);
...
\begin{frame}{ outcome paths, three policy settings}%: 
...
\begin{tikzpicture} 
\node at (.7cm,10cm)
{\includegraphics[scale=.78, trim={0 1.4cm 4cm .5cm},clip]{fig/cg_tax_invest-Main-011821.pdf}};
...
\item This is the no linkage case. Notice that changing the capital gains tax on investors does not affect `real' variables
...
\node at(8.5, 4.98)[above left] {\huge Higher wages};
...
\draw[red!50,line width=1mm] (6.5,3.9) circle (1.55cm);
\draw[latex-,line width=1mm  ] (7.5,3.5)--+(.5, 2); 
\node[above]at(7,3.7 ){\huge Higher rate on owner-occupiers};%};\Huge\$74,000};
