...
\usetikzlibrary{chains, fit, shadings, shadows, shapes, shapes.geometric, arrows, calc, positioning, shapes.geometric, arrows.meta, patterns, patterns.meta, decorations.text, decorations.pathmorphing}
% for flow diagram:
...
plot2/.style={thick, smooth, samples=100, domain=0.1:14.99},
 ...
 \draw[my axis] (0,13)node[left] {$Y=Fy$} --(0,0)-- (15.4, 0) node[below left] {Urban population $N=Fn$}; %creates the axis a little 
...
\vspace{.2cm}
\small  
$N =$ population, $n=$ firm workforce, $F=$ number of firms, $ N = F n$, 

 $Y=$ city GDP, $y=$ firm output, $Y=Fy$, $\gamma =$ agglomeration effect,

$Y=Fy$  exhibits increasing returns to scale if $\gamma+\alpha+\beta^{firm}>1$.
% $Y= FAF^\gamma n^\gamma z^\alpha n^\alpha n^{\beta_{firm}} = A(Fn)^{\gamma+\alpha +\beta_{firm}}$ where $A=A^{firm}F^{1-\alpha-\beta}z^\alpha$, and  $k=zn$
...
\only<2>{
\draw[red, line width=.5mm] (-2.2, 4)  rectangle (2,1.5);
...
\node at(0,4.5){\includegraphics[scale=.6,trim={1.cm 1.cm 1.cm 1.cm}, clip]{fig/flow-alonso-jacobs-cycle.png} };
% \vspace{-.5cm}
 \node at (-3,1.5){ALONZO}; 
 \node at (3,1.5){JACOBS};
%\draw (0,0) circle (1cm);
...
\draw[green, line width=.5mm, ](-.5, 1.5) rectangle (3.2,-.7);
\node at (5.25, 1.27)[green]{\Large Finance/lending};
...
\centering
\begin{tikzpicture}
\draw[green, line width=1.5mm](-5,1) rectangle (5,8);
\node at(0,4.5){\includegraphics[scale=.175,trim={2.75cm 2.75cm 2.75cm 2.75cm}, clip]{fig/flow-financialization.png} };
%\draw (0,0) circle (1cm);
\end{tikzpicture}
%, fill=gray
...
%%%%%%%%%%%%%%%%%%%%%.%%%%%%%%%.%%%%%%%%.%%%%%
\begin{frame}{Distributional model of housing market with three classes}
...
% \end{frame}
%%%%%%%%%%%%%%%%%%%%%.%%%%%%%%%.%%%%%%%%. SAY 
\begin{frame}{Program scheduling}
...
\draw[latex-,line width=1mm  ] (5.8,4.5)--+(.5,.5)node[above right]{\huge Wages lower};%};\Huge\$74,000};
...
\draw[latex-,line width=1mm  ] (-3,-3.6)--+(1,-.8)node[ right]{\huge Crossover};
\end{tikzpicture}
}
\end{frame}% 
