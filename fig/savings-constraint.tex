...
% COMBINED MAX RED
\draw[ultra thick, red, opacity=.85] (0,0)--(2.5,2)--(4.0,2)node[right]{\small Borrowing constraints};
\node at (4.0,1.95)[below right, red, opacity=.85]{\small $M_i^{max}=min(M_{Y_i}^{max},M_{W_i}^{max})$ };

\draw[ultra thick, blue, opacity=.85] (0,.7)--(2.55,2.7)--(4.2,2.7)node[right, ]{\small Savings constrained bid}node[below right, ]{$M_i^{max}+S_i$};
% SAVINGS
