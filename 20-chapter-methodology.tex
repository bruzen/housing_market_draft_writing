...
We use an agent based model of the housing market. The agent based modelling approach developed in the 1940's when von Neumann and Ulam examined cellular automata, a simple form of agent model where agents are typically squares on a grid following rules based only on the state of their neighbours \cite{banks_statistical_2009}. ABMs %provide a disaggregated way to understand the structure of models. They 
...
Our model of firm behaviour uses modelling of representative agents. {\color{red}CLARIFY WHAT PARTS OF OUR MODEL ARE NEOCLASSICAL/CLASSICAL HOW}
This allows us to focus on the relationship between the scaling of urban productivity and the financialized ownership that can extract that value, which is  the core conceptual contribution of this work. 
...
% (MAYBE ADD THIS? Note:
We do not assume equilibrium conditions in the agent model of the housing market. Combining a spatially explicit agent based model of the housing market with a model of rent and urban production allows us toto stay close to the analytic tradition, relaxing assumptions to clarify what drives each results, and connect the work with classical and neo-classical theory. % \dots)


\section{Equilibrium reasoning in our analysis}

One feature of this approach is thus to combine agent based analysis with an agent based model. 
...
processes, and economists, like other modellers, now very often resort to simulation and numerical methods. 
...
REPHRASE - NEED SOMETHING LIKE THIS (FROM THE INTRODUCTION) HERE `We thus combine a spatially explicit agent based land market model, with an equilibrium assumption about rent and an equation based model of urban production. By integrating these two pieces which are traditionally studied separately, this work combines elements  of classical rent theory and neoclassical distribution theory with agent based modelling. '
