...
Our approach/model is constructed, drawing together pieces from a number of sub-literatures from economics and the study of cities, including rent theory, production functions, the standard urban model, growth theory, urban growth theories, financialization and the theory of distributional. 
...
The production economy is what generates a surplus.  This means we have a production/production economy. %, while many models of wealth distibution look at capital and financial markets, 
%Demand for the urban product is perfectly elastic and the price per unit is 1. 
We consider firms selling into a large/comodity market, so demand is elastic, and selling more won't affect the product's price. %This means each new worker increases output by less than the last worker did,
There are decreasing returns to scale. The marginal product of labour declines monotonically. There is no frictional unemployment, and no labour adjustment costs.   % Note in the agent model - employees are simply laid off and seek work, so there is unemployment, but not labour adjustment costs for firms.
