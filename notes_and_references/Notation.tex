...
Labour supply, the number of workers, which, in the standard circular city model, equals the number of lots of size $s$  when workers live on identical individual lots. % Unless $d^{max}>d^*$
\\  \hline
...
$\sigma$ (or considered $\chi \Gamma$   \rotatebox[origin=c]{180}{$t$} \reflectbox{$t$})  &  Property tax share  \\
\hline
\color{black}
\end{longtable}  
