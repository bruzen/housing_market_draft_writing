...
}{Johanna Palberg \cite{palmbergSpontaneousOrdersEmergence2013}}
...
We're modelling how financialization captures the value created in cities. To show how it does this, we need %to show what it is that is being captured and it is the growth of productivity that is captured. 
 a model of growth that can show how value is created and concentrated in cities. %, and the value that can be captured through land in cities through financializaiton.
...
In \textit{The Economy of Cities}, Jacobs \cite{jacobsEconomyCities1969} argued that %put forward a compelling suggestion: %Agglomeration is here mechanism: %in response to an important question in economics:  
...
Scaling analysis is a tool developed originally in \glspl{complex system} science to investigate how \glsdisp{extensive property}{extensive properties} of the system vary with a system's size,  or `scale.' Our particular interest is in studies linking city population and economic output, but in urban science, it has been used in many studies of the relationships between urban population size and features like urban economic output, area, growth, traffic congestion costs, and even social indicators like crime and homicide rates. These studies provide insight into how these processes are affected by the size of a city. 
...
Three general observations about urban scaling appear to be widely accepted:
\begin{enumerate}
...
% \vspace{.25cm}
 ...
 % \hspace{-1cm} 
\item the relationship appears to have applied well back into pre-history \cite{westScaleUniversalLaws2017}, from the smallest human communities to modern megacities, %{\color{red} ADD CITATIONS this is in urban science and West's scale, though there's a paper they cite..} % It is not a function of any government, economic, or cultural form. It's not simply modern cities or capitalist cities. 
\item scale effects derive from the capacity of people to interact (as Jacobs suggested). \cite{haryantoRelationshipUrbanizationEducation2021}%{\color{red} ADD CITATIONS}
...
\end{enumerate}
These scale effects are the same as the agglomeration effects long discussed in the economics literature. %Empirical work on scaling reveals that  
%the super-linear scaling of wealth in cities is one of the most persistent stylized facts in economics.
...
\input{fig/production_functions}
    \caption[Increasing returns to scale for cities and decreasing returns for firms]{ Although each individual firm exhibits decreasing returns to scale, with optimal labour force $n_R$ and output $y_R$, the city population as a whole, $N$, yields increasing returns $AN^\beta$ if the exponent $\beta>1$, illustrated by the green line. Increasing returns produces an agglomeration surplus, $AN^\beta-\frac{N}{n_R}y_R$, where $\frac{N}{n_R}$ is the number of optimal firms at $N$, and $y_R$ is the output of an optimal firm.}
...
If we take the relationship implicit in her work, and write it in the language of modern growth theory, %{\color{red} He WAS CONFUSED ABOUT T HERE 'What is T? Same as above in $R^Tr$?'} 
we get:
...
If we use consider a fixed capital-labour ratio $d$ as Arrow does,  capital used by the firm is  $K=d*L$ and Equation~\ref{eqn-production-jacobs-agg} becomes %{\color{red} COMMENT - WHY SUDDENLY USE *?}:
\begin{align}
...
\centering
%               social  exp err nation   obs    year    admin  ref         
\begin{tabular}{|p{1.5cm}|c|l|p{1.25cm}|p{1.5cm}|p{1.cm}|p{1.5cm}|p{2.5cm}|}\hline
...
Agglomeration effects are the third piece needed to build a model relating urban productivity and the financialization of the urban housing market. % lucas saying---you knowo what jacobs was probably right, that's probably what we've come to with all this work (Lucas is a big  name.)
...
We model the agglomeration effects by embedding an urban \gls{scaling law} to model how value is created in cities. %to develop the theoretical grounds of our model, we 
