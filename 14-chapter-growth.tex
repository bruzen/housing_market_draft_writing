...
\epigraph{Cities are the engines of economic growth (Jacobs, 1969; Bairoch, 1988). It is in cities that a large share of the innova- tions and entrepreneurship takes place that fosters economic growth in the long run. Spontaneous Orders and the Emergence of Economically Powerful Cities.}{JOHANNA PALMBERG}
...
The model so far is a static   structure it doesn't have any growth. 
the fundamental feature of city is that they grow add they are creative as destified by their permanentc and their growth over millenai. 

in order to understand what's at  stake  we have to be able to desciribe to identify that growth

the hypothesis is that the  city grows, a big chunk appears as land value and finance is grabing it.

To formally model this we need to incorporate one final  piece i nto  ouor model,  growth. 

In this chapter we introduce the structure of growth, giving the complete base model.

Agglomeration is what drives the growth in the jacobs modoel.

What a re the forces that leads to wealth -- growth in land values and therefore the opportunities for extracting wealth.  What are thosoe basic forces? 
...
But what is the source of economic growth and innovation that has characterized human civilizations? In The Economy of Cities (1969), Jane Jacobs \cite{jacobsEconomyCities1969} put forward a compelling suggestion. Agglomeration is here mechanism: %in response to an important question in economics:  
cities are the primary drivers of economic development, and the mechanism that explains growth is  the growing opportunities for sharing and creating ideas as population and population density increase. The \gls{Jacobs model}\footnote{Glaeser et al. \cite{glaeserGrowthCities1991} refer to the Jacobs model as the Jacobs-Rosenberg-Bairoch \cite{bairochCitiesEconomicDevelopment1988, rosenbergTechnologicalChangeMachine1963} model.}  both describes the effect of urban agglomeration and explains it.

 Jacob's argument is  essentially that the density of cities produces an agglomeration effect that drive economic growth.  
...
He's pointing  to agglomeartion economies as the force that holds cities

what takes them appart is the transportation effect. 

So we have to have the aglomeration effect which producesd the agglomation effec twhich produced the corcumstances that genreates wealth within the  model.

We can't model what we care about with these cities  without modoeling what dirives them/what hold them toobether- according to lucas i t is the aglomareatin effects which offset all the forces of idispersion.

In this model we develop the theory to modoel growth and aggloomatioon in our urban model with rents.


Lucas is saying there is some mysterisous force that overcomes all the reason's people have too spread  out. 

Fundamentally it is the productiveness when people come together.





Lucas' observation provides a natural link between the Jacobs mode, which is based on urban agglomeration, the neoclassical growth model operating at the level of nations, and the scaling literature on cities. In this chapter, we link our basic spatial model to the scaling of urban productivity,  as Lucas seems to be suggesting, and in the process,  we will provide the foundation for our model of urban agglomeration effects.

\section{Cities and the scaling literature}

This is  there. its's called an aggloermation econoomy emprically and theres strong empirical eveidence it holds storngly and a whole range of theoretical explanations.

it followos this empircal results.

(growth theory  and aglgomeartion appraoches-- which apeare  to be fundamentally diffeerent actuallyt produce the same quation, .--)

dod a bunch of equation to show how one can be turned into the other.. 
WHENEVER TALKS ABOUT ONE - LOOKS - WHENEIVER DIFFERENT INTIALI POINT AND COME FROM DIFFERENT PLACES the growth  theory all comes otu of lookign a t gdp growht and using the cob douglass to use gdp growth.
The agglomeraiton results come out of straight empirical hacking.
when xyz and bettencourt go back to ohow you could get htis equation they go back to the cobb doouglass and that same analysis. -- that's  the loogic of that whole chapter
one can be turned into the other.

all this relationship on why we get growing procductivithy and it comes to  a certain relationship
relationship seems to say its human c apitla not just the  number oof workers its their productivity

bring together, can specialize, can get more education, can learn from another
these are all agglomeration effects, though t hey don't call them agglomeration effects

lucas saying - you knowo what jacobs was probably right, that's probably what we've come to with all this work
(Lucas is a big  name.)
...
The history of economic thinking about agglomeration effects goes back at least to  Alfred Marshall in 1890 \cite{giddingsPRINCIPLESECONOMICSAlfred1890}. % {\color{red}
...
Given that there is such a strong theoretical and empirical grounding for urban agglomeration effects in the city, we build our model of the housing market on top of a model of the urban system that exhibits positive agglomeration effects. %***E EDIT THIS. TO CLARIFY RELATIONSHIP BETWEEN SENTENCES. BUILD AGGLOM EFECT INTO MODEL BY EMBEDING SCALING LAW? OR SPECIFICALLY WE EMBED??? 
