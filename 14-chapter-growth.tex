...
% \vspace{.25cm}
 ...
 % \hspace{-1cm} 
\item the relationship appears to have applied well back into pre-history, from the smallest human communities to modern megacities, {\color{red} ADD CITATIONS this is in urban science and West's scale, though there's a paper they cite..} % It is not a function of any government, economic, or cultural form. It's not simply modern cities or capitalist cities. 
\item scale effects derive from the capacity of people to interact (as Jacobs suggested). {\color{red} ADD CITATIONS}
...
\input{fig/production_functions}
    \caption[Increasing returns to scale for cities and decreasing returns for firms]{{\color{red} ADD LABELS FOR VARIABLES IN CAPTIONS} While each individual firm exhibits decreasing returns to scale, the city as a whole exhibits increasing returns to scale, and thus produces an agglomeration surplus, $AN^\beta-\frac{N}{n_R}y_R$.}
...
The evidence supports the existence of significant external learning spillovers in a variety of industries. Using survey data, 
...
If we take the relationship implicit in her work, and write it in the language of modern growth theory, {\color{red} He WAS CONFUSED ABOUT T HERE 'What is T? Same as above in $R^Tr$?'} we get:
...
\textbf{Social Rates} & \textbf{Exponent} & \textbf{Error} & \textbf{Nation} & \textbf{Obser-vations} & \textbf{Year} & \textbf{{\color{red} HE HAD A QUESTION MARK HERE FOR SOME REASON Unit}} & \textbf{Reference} \\ \hline   
