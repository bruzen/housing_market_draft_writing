...
{\color{red}
The model so far is a static structure. It doesn't have any growth. The fundamental feature of cities is that they grow and they are creative as demonstrated by their permanence and their growth over millennia. In order to understand what's at stake in cities, we have to be able to describe that growth.
Our hypothesis is that the  city grows, a big chunk appears as land value, and finance is capturing grabbing it. We have model of rent and space. %To formally model this we need to incorporate 
We need one final  piece: growth. In this chapter we introduce the structure of growth. %, giving the complete base model. 
% Agglomeration is what drives the growth in the Jacobs model.

% What are the basic forces that leads to wealth: to the growth in land values and therefore to the opportunities for extracting wealth.  %What are those basic forces? 
}
...
Lucas is saying there is some mysterious force that overcomes all the reason's people have too spread out. Fundamentally it is the productiveness when people come together.

Lucas is pointing  to agglomeration economies as the force that holds cities together. 
What takes them apart is the transportation effect. 
...
We can't model what we care about with these cities  without modeling what %drives them/what 
holds them together. According to Lucas it is the agglomeration effects which offset all the forces of dispersion.
In this model we develop the theory to model growth and aglomeration in our urban model. % with rents.
}
