...
In developing the theoretical model we introduced a number of rates, such as $r_i$, the rate that  individual $i$ pays for a single period of borrowing. The payment calculation is made for a period of length $T$, which we refer to as a mortgage term.
This means the actual interest paid is the compounded interest rates (VARIABLES LIST FOR THIS).
