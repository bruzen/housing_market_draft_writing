...
In the following sections we discuss the computation of bid parameters, outline potential discounting approaches, and develop illustrative values for parameter values.
...
In the following subsections, we discuss each of the terms and how they are implemented in the code. Net rent, $\mathcal{R}_N$, is discussed in subsection \ref{SS:NetRent}. The borrowing ratio, $m$, is discussed in section \ref{SS:BorrowingRatio}. The discount factor, $\delta$ is discussed in section \ref{SS:discountfactor}. The price approximation mechanism, $L(P)$ is discussed in \ref{SS:PriceForecast}, and the bank interest rate, $r$ is discussed in \ref{SS:BankRate}. Sections are thus numbered: 

\begin{align}
P^{bid} \ge   \frac{\ref{SS:NetRent}} {(1-\ref{SS:BorrowingRatio})\ref{SS:targetr}-
