...
We can initialize the model with an arbitrary  $N$ and the workforce size of the rural firm, $n_R$. From this, using the production function we  calculate the both output and the marginal product of labour,  $MPL_t$.
...
%    Y_t=& N_ty^\gamma y^\]  
\end{equation}
$\gamma$ is drawn form the agglomeration literature.% but $A$ must be calculated to.
...
\end{equation} . 
Firms are assumed to set the mPL equalt ot the wage, $\omega+\psi$. This is used to calculate target employment. As $N$ rises the marginal product of labour rises, so the firm will want to hire more workers and is willing to pay a higher wage.

Firms observe their output, ${y}_t$ and find their $MPL_t$. They  adjust labour purchase so that the $MPL_t$ is equal to the wage.\footnote{This is a prescription from the basic theory of the firm for maximizing profits.} 

\subsubsection{Urban firms' marginal product of capital} 
Firms also adjust their capital stock as the relative price of labour rises. We can compute the marginal product of capital in the same eway we calculated the marginal product of labour, by differentiating the output equation with respect to capital]:
...
If  $MPK_t > r_F$  firms are making a profit and standard micro theory tells us new firms will enter the market $dF/dt >0$. The converse is true as well: $MPK_t > r_F \rightarrow dF/dt <0$. Theory tells us nothing about how many, but suggests that this adjustment will be slower than the adjustment of the labour market target adjustment rate, $\frac{n^{target}_{t+1}}{n_{t}}$. If we (arbitrarily) assume the rate is 0.25  of the target growth rate of the labour force,
% \begin{equation}
