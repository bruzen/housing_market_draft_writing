...
\begin{equation}MPL_{t} = \frac{\beta_{F}{y}_{t}}{n_t}\label{eqn:MPL}\end{equation} 
...
\begin{equation} MPK_t = \frac{\alpha_{F}{y}_{t}}{k_{t}}\end{equation} 
...
Aggregate urban labour demand for the following period has two parts--the demand of existing firms and the demand of new entrants. In each case, the firms sets hiring targets, it may or may not achieve.
\[N_{total,t+1}^{target}= N_{t+1}^{target}+N_{new, t+1}^{target} \]

 We begin by calculating targets. 
...
Firms also find their marginal product of capital, $MPK_t$ which should equality the cost of capital, $r_F$. % and adjust the capital stock so that the MPK$_t$ is equal to $r_F$. 

\begin{equation}k_{t+1}^{target}= \frac{\alpha_{F}{y}^{target}_{t}}{r_F},\end{equation}



using a new output, $y_t^{target}$, which is calculated using $n^{target}_{t+1}$. % into Equation~\ref{eqn-urban-firm-output}.
...
\begin{equation} wage_{exist,t+1}= (1-adj_{existing}) \omega_{exist, t-1} + adj_{exist} wage_{exist,t}  +\psi, \end{equation}
