...
Assuming that a buyer always takes the maximum mortgage available, maximizes the financial return on the transaction. % That means that the  

With this assumption, the maximum price a person can bid is:
% RULE 2 The maximum price that one can bid is $ P_i^{max}=M_i^{max} + \mathrm{savings_i}$
...
Equation~\ref{eqn-property-investment-return1} gives the rate of return for a property, rented out. An investor will invest if the expected return on their investment is greater than their target rate of return, % that the investor can price offer for the property and still satisfy
