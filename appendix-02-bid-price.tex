...
To find how much investors will bid, we need to find the maximum price that they are willing to pay.  This is the maximum price that satisfies the criterion $r^{return} \geq r^{target}$.  The bid price is used in the price determination process in our model. %Where the return is the value over the down payment:
...
We start by writing Equation \ref{eqn-property-investment-value1} with the net rent, which can be written a share, $\phi$ of total rent, %The net rent can also be written as a share, $\phi$, of the rent 
%$\mathcal{R}_N = \phi \mathcal{R}$: %(originally from Chapter \ref{chapter-financialization} on Financialization. Since the rent is a known quantity at the beginning of the term, independent of the current, so we reformulate net rent,  $\mathcal{R}$, $\mathcal{O}$, $\mathcal{T}$ as a fraction of total rent: 

\begin{eqnarray}
V &=& \delta \left(P_T - (1+r)M\right) +      \mathcal{R}_N   \nonumber \\
  % &=& \delta \left(P_T - (1+r)M\right) + \phi \mathcal{R}   \nonumber \\
  % &=& \delta \left(P_T - (1+r)m P_0\right) + \phi \mathcal{R} \nonumber \\
  &=& \delta \left((1+\dot P)P_0 - (1+r)m P_0\right)     + \phi \mathcal{R} \nonumber \\
  &=& \delta \left((1+\dot P)    - (1+r)m    \right) P_0 + \phi \mathcal{R}
...
\begin{eqnarray}
r_{return} = \frac{\delta \left(1 + \dot P - (1+r)m\right)}{1-m} + \frac{\phi \mathcal{R}}{(1-m)P_0}
\end{eqnarray}
...
The rule is, ``Bid if the RHS is larger than the target rate of return, $r^{target}$, and do not bid if it is smaller'', as defined in Equation \ref{eqn-property-investment-return2}.  The maximum bid is thus the bid that makes the two sides equal in the following equation, the bid gives a return greater than or equal to the target return rate. Replacing $r_{return}$ with $r_{target}$, $P_0$ with $P^{bid}$, and $\dot P$ with an estimator for $\dot P$, $L(P)$, we get TODO REPLACE PDOT - MAYBE WITH TILDE AND DOT..:  %$\dot \tilde{A}$ $\wwtilde{\dot{u}}$

\begin{eqnarray}
r_{target} \ge \frac{\delta \left((1+ \dot P - (1+r)m\right)}{1-m} + \frac{\phi \mathcal{R}}{(1-m)P^{bid}}
\end{eqnarray}
...
\begin{align}
r^{target} &\ge \frac{\delta \left(1 + \dot P - (1+r)m\right)}{1-m}   +\frac{\phi \mathcal{R}}{(1-m)P^{bid}}. \nonumber \\
(1-m)r^{target} &\ge \ \ \delta \left(1 + \dot P - (1+r)m\right) + \frac{\phi \mathcal{R}}{P^{bid}} \nonumber \\ %\delta(1+L(P))- (1+r)m%
(1-m)r^{target} - \delta \left(1 + \dot P - (1+r)m\right)  &\ge  \frac{\phi \mathcal{R}}{P^{bid}} \nonumber\\
P^{bid} & \le    \frac{\phi \mathcal{R}}{(1-m)r^{target}-\delta \left(1 + \dot P - (1+r)m\right)} \\
P^{bid} & \le    \frac{\phi \mathcal{R}}{(1-m)r^{target}-\left[ \delta(1+L(P)- (1+r)m\right]} \nonumber 
\end{align}
