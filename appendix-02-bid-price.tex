...
This chapter links the theoretical discussions of the previous chapters with an agent-based simulation model. The chapter %is intended to explain the code and justify the modeling decisions that link the theory to the actual 
...
In each period a house offer two kinds of services - home services proper and locational services - access to the central city job. We call this the \glspl{warranted market rent}, and we expect a tenant to pay this amount.

Homes must be maintained and taxes paid, so an owner would enjoy only the net rent,   $\mathcal{R}_N$

The \gls{warranted market price} of a house then is the present discounted value of the flow of those service.
\begin{equation}
  P^w=\frac{\mathcal{R}_N}{r}  
\label{eq:Capitalization}
\end{equation}
where $\mathcal{R}_N$ is the net value of rents, and $r$ is the  interest rate.
We expect it to be the sale price of a unit of housing with land in the absence of potential capital gains.

\subsection{locational services}

To formalize the description, locational services are, on an annual basis $\omega- {c} * d$. A property at $d$ offers the combined annual services of 

\begin{align}
h=\omega- {c} * d + a\psi  
\label{eqn-housing-price}
\end{align}
where $a$ is a fraction of the subsistence income , $psi$. Annual rent charged initially should be  equal to the value of services, $h$:
...
Actual rents do diverge from warranted rents, but they are limited by what tenants can pay, and that in turn is limited by urban wages. When we assume an subsistence wage $\psi$ with a specific share $a$ 
...
\[ PDV(P_M^e_T)=P_0\left( \frac{1+\dot P}{1+r_\delta} \right)^T \]
