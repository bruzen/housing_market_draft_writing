...
V &=& \mathcal{C} + \mathcal{R}_N  \label{eqn-property-investment-value0}\\
  ...
  &=& \delta \left(P_T- \left(1+r\right)M\right) + \mathcal{R} - \mathcal{O} - \mathcal{T}.\label{eqn-property-investment-value1}
\end{eqnarray}
...
To find how much investors will bid, we need to find the maximum price that they are willing to pay.  This is the maximum price that satisfies the criterion $r_{return} \geq r_{target}$.  The bid price is used in the price determination process in our model. %Where the return is the value over the down payment:
...
%We return to Equation~\ref{eq:Value} %
We start by modifying Equation \ref{eqn-property-investment-value1}, (originally from Chapter \ref{chapter-financialization} on Financialization.
Since the rent is a known quantity at the beginning of the term, independent of the current, so we reformulate net rent,  $\mathcal{R}$, $\mathcal{O}$, $\mathcal{T}$ as a fraction of total rent: 
...
Since the  price at the end of the term $T$,  $P^T$, is a predicted value  for the investor \textit{ex-ante}, so we replace $P^T$ in Equation~\ref{eq:Value} with a predictor, $(1+\dot P)P^{bid}$. 
