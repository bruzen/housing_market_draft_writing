...
The agent will only sell if it's worthwhile, that is if some minimum bid is reached. To include this minimum simply, we %It might be simplest to 
treat every seller as a bidder. All bids above her own are considered. The seller chooses the second highest bid. %This  seems realistic enough and is very simple to implement. It should producer a path that is indistinguishable form any more complex approach. 
...
${c}$            &  Transportation cost per unit distance \\ % was $\tau$, considered $\gamma$, $\xi, \zeta$
...
Values related to time are superscripts, time as a continuous variable is small, a period is capitalized e.g. the period $T$ of some number of years. 
Agent counts and indices are subscript.
