...
Housing services absorb about 30\% of income  we will use than number  as an approximation,  $a\phi$, where a is the share of the subsistence wage and $\phi$
...
\[\mathcal R^{charged}= h= \omega- trans * d + a\phi]\]
...
In each price a period a `mill rate'. Each mill is  expressed as  1/1,000 of the value is determines - The annual tax is initially
 ...
 $(taxrate)*P_0=(millrate)*\frac{\omega- trans * d + a\phi}{r}$
...
b  = share of share of subsistence wage  used on maintenance

c  = annual tax rate on rent and home  e,g.12 mills
\end{quotation}
Now Net Warranted market  rental price is
\begin{align}
\mathcal{R}_N^w &= \omega - trans*d + a\phi -  \Theta - \Sigma \\
\end{align}

It is also Net Warranted market  rental price that is relevant in taxation and maintenance. 

\begin{align}
\Theta &\equiv  ba\phi\\%Maintenance?
 \Sigma &\equiv ca\phi %taxes?
\end{align}
so we can write
 \begin{align}
\mathcal{R}_N^w &= \omega - trans*d- + a\phi -   ba\phi - ca\phi \\
\end{align}
...
\subsection{How does this affect the bid price?}
...
\begin{align*}
r_{return} = \frac{\delta \left(1 + \dot P - (1+r)m\right)}{1-m} + \frac{\mathcal{R}_N}{(1-m)^{bid}}\tag{B7}
\end{align*}
...
P_i^{bid} \le &   \frac{\mathcal{R}^w_N}{(1-m_i)r_i^{target} - \delta_i \left(1 + L(P) - (1+r_i)m_i \right)} \tag{B10}\\
		 \le &   \frac{ \omega - trans*d- + a\phi -   ba\phi - ca\phi}{(1-m_i)r_i^{target} - \delta_i \left(1 + L(P) - (1+r_i)m_i \right)}		
\end{align*}
