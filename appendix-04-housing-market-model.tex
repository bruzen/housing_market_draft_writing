\chapter{Bids and reservation prices}\label{sec:bids-and-reservation}
...
Retirees, i.e., individuals who reach the end of their working lives and no longer need access to the central city, may put their homes on the market, making room for new workers to enter the urban market.\footnote{For owner-occupiers, the stream must be discounted at the personal discount rate or at the personal borrowing rate.}  %\footnote{The timing decision can be randomized, but that is highly unlikely to affect results.}
...
\subsubsection{5. Retiring tenants who  move to the country} 
...
This requires that the sum of  locational rent, net amenity and net capital gain be greater than or equal to zero and greater than $\mathcal{R_M}$ for the in-migrant to buy.
 ...
 %People also make choices with imperfect information, some are investing in living in a city


% \begin{align}%  MORTGAGE CONSTRAINT ON MAXIMUM BID
...
% \[{r}(P^{realized}-M) >\ a\psi+ \mathbb{A}^{net}\label{eq:movers-gainA}\]

% This value provides a \textbf{reservation price}: 
% \[P^{reservation} =\ \frac{a\psi+ \mathbb{A}^{net}}{r}+M \label{eq:movers-gainB}\]
...
% \[\mathcal{R}^{net}>{r}(P^{realized}-M)\]
% This value provides a \textbf{another reservation price}: 
% \[P^{reservation} =\ \frac{\mathcal{R}^{net}}{r}+M \label{eq:movers-gainC}\]
% }
...
\subsubsection{The cost of delayed sale}
 If a sale is delayed by one period, however, the differential gain is deferred, so the present discounted cost of waiting must be weighed against the chance of a higher price in the next round. %$Gain*\frac{\delta}{1+\delta}$.????%\footnote{This is simply (1 minus the discount factor) times the deferred amount.} 
...
%My suggestion is that as an intitial step, if the property seller does not sell the owner reduces the reservation price for the next period by (say) 5\% . The owner could use the expoected price as the initial list price. 
...
% %\begin{eqnarray}
% \[P_{bank}^{reservation}  \le  P_{person}^{max\_bid} \]
