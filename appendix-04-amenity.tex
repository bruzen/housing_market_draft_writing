...
\begin{equation}
U(w,d)= w+ \omega+ A(d; n) - T(d))\label{Eqn:U}
\end{equation}
where $w$  is an urban wage premium, $T(d)$ is transportation cost from the centre to $d$.
\footnote{\cite{AnasArnottSmall98} show that a linear transportation cost will not  hold if congestion declines  with $d$.} %and $H(d)$ is the physical house cost independent of land value at $d$. WE assume 
