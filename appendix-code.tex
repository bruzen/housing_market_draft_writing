...
If we assume that the rental rates actually charged are the warranted economic rent, Net rent is
$\mathcal{R}_N = (1-\kappa_j - \sigma_j) (\omega-tau*d_j)$


The tax ratio $\sigma$ is an important number - it is the share of rents that the municipality takes for services and infrastructure. I estimate it to be about 30\% based on Mill rates in Ontario,  so $\sigma=0.3$.


In reality, $\kappa$  varies for every property based on maintenance requirements, historic rents, tenant rights, and variations in assessed values. This will matter for the buyer. If it varies may be useful as a quality indicator.
...
\[\mathcal{R}_N^T= (\omega-\tau*d_j)\sum_{\tau=0}^{\tau=T-1} \frac{(1+\dot{rent})^{\tau}} {(1+r_{discr_i})^{\tau+1}} \]

% \[\mathcal{R}_Nj^T= (\omega-\tau*d_j)\sum_{\tau=0}^{\tau=T-1}\left[\frac{1+\dot{rent}}{1+r_{discr_i}}\right]^\tau \]
