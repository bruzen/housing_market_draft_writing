...
param: \textbf{money cost of time per km year} =time cost per km-year* wage(including subsistance = 0.012 year* wage per year
...
These can be combined into a funtion %\deltathat  gives  a single discounting factor  for a value  like future price that is both growing and being discounted over several (T) periods:
...
So I want to use this relationship to find the maximum bid price for the bank. The rule is, ``Bid if the RHS is larger than the target rate of return, $r_{target}$, and do not bid if it is smaller.''  The maximum bid the bid that makes the two sides equal. 
\begin{eqnarray}
...
\foreach; \yi in {0,2,4,6,8,10} \draw (0,\yi)--(-.1,\yi)node[left]{\small$\yi$};
\draw[func,domain=\Wmin:\W] plot [samples=200] (\x,{(9-\x^.5/2});

% \def \A {.8}
% \draw[func,domain=\Wmin:\W, green] plot [samples=200] (\x,{(\A+\B*\Wbar/\x)*\rbar});
...
(I found an easier equation to work with below)

I think $\bar r$ is just the bank rate set by the Bank of Canada. Exogenous. Just define it. It could be the bank's target rate, prime plus 1\%, say.
