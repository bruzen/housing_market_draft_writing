...
\item The second is my modelling decision about how to connect the production sector to the urban system in my model
...
\begin{center}
 \vspace{-.5cm}   
\includegraphics[scale=.55,trim={.5cm 1cm 0 1.5cm},clip]{fig/flow-impacts.png}
\end{center}
...
\item We model each sale as a multi-bidder auction.
...
% %----------------------------%
\begin{frame}{CASE: Capital gains tax on investors with no linkage}
\begin{tikzpicture} 
\node at (.7cm,10cm){
...
\item a capital gains tax does not affect production side but does affect the ownership ratio profoundly.
...
\end{frame}% %
\note[itemize]{\Large
\item comparing the case with a strong linkage to productivity 
\item effect of capital gains tax on ownership is larger
\item \textbf{With linkage}  the CGT improves all production-side variables and increases city size
\item a capital gains tax does not affect production side but does affect the ownership ratio profoundly.lower than the cgtax for owners-occupiers leads to a population crash
